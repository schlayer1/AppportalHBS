...
```mermaid
graph TD
    User([Lehrkraft / Smartboard / Schüler]) --> PWA[PWA Web-Shell / React 18]
    PWA --> Cache[(Service Worker Cache-First / Offline)]
    PWA --> Store[LocalState & Resilient Storage]
    PWA --> LocalNet[BroadcastChannel API / Sub-ms Peer Sync]
    PWA --> CloudSync[Firebase Cloud Firestore / Realtime Snapshot]
    PWA --> WebAudio[Web Audio API Procedural Synth]
```
